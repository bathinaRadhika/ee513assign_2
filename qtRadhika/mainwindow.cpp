    ...
    ui(new Ui::MainWindow)
{
    ...
    this->count = 5;
    this->time = 0;
    this->setWindowTitle("SUBSCRIBER");
    this->ui->customPlot->addGraph();
    this->ui->customPlot->yAxis->setLabel("Pitch values ");
...
void MainWindow::update(){
    ...
    QString text = QString("Recent value is %1").arg(this->count);
...
void MainWindow::on_connectButton_clicked()
{
   ...
   // int x = MQTTClient_subscribe(client, TOPIC, QOS);
    ...
    int x;
     x = MQTTClient_subscribe(client, "ee513/#" , QOS);
...
void MainWindow::on_MQTTmessage(QString payload){
    ...
    //ADD YOUR CODE HERE
      ...
      QJsonObject sample = obj["ee513"].toObject();

      float temp =(float) sample["CPUTemp"].toDouble();
      float time= (float) sample["CPUTime"].toDouble();
      float roll= (float) sample["roll"].toDouble();
      float pitch = (float) sample["pitch"].toDouble();

      if(ui->pitch->isChecked())
      {
          QString text = QString("pitch value is %1").arg(pitch);
          ui->outputEdit->setText(text);
      }
      else if(ui->roll->isChecked())
      {
          QString text = QString("Roll value is %1").arg(roll);
          ui->outputEdit->setText(text);
      }
      else if(ui->time)
      {
          QString text = QString("Time value is %s").arg(time);
          ui->outputEdit->setText(text);
      }
      else
      {
          QString text = QString("temperature value is %1").arg(temp);
          ui->outputEdit->setText(text);
      }
